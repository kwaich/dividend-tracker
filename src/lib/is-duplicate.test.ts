...
describe("isDuplicate", () => {
  ...
  it("returns true for same symbol with different asset id when fallback is enabled", () => {
    expect(isDuplicate("aapl-lse", "AAPL", baseDateMs, "acct1", existing)).toBe(true);
  });

  it("returns false for different symbol", () => {
    expect(isDuplicate("msft-us", "MSFT", baseDateMs, "acct1", existing)).toBe(false);
  });

  it("returns false for different account", () => {
    expect(isDuplicate("aapl-us", "AAPL", baseDateMs, "acct2", existing)).toBe(false);
  });
  ...
  it("falls back to case-insensitive symbol matching when existing assetId is missing", () => {
    ...
    expect(isDuplicate("aapl-us", "AAPL", baseDateMs, "acct1", withNull)).toBe(true);
  });

  it("handles multiple existing activities correctly", () => {
    const multi = [
      makeActivity({ assetId: "msft-us", assetSymbol: "MSFT", accountId: "acct1", date: baseDate }),
      makeActivity({ assetId: "aapl-us", assetSymbol: "AAPL", accountId: "acct2", date: baseDate }),
      makeActivity({ assetId: "aapl-us", assetSymbol: "AAPL", accountId: "acct1", date: baseDate }),
    ];
    expect(isDuplicate("aapl-us", "AAPL", baseDateMs, "acct1", multi)).toBe(true);
    expect(isDuplicate("aapl-us", "AAPL", baseDateMs, "acct2", multi)).toBe(true);
    expect(isDuplicate("msft-us", "MSFT", baseDateMs, "acct1", multi)).toBe(true);
    expect(isDuplicate("msft-us", "MSFT", baseDateMs, "acct2", multi)).toBe(false);
  });
});
